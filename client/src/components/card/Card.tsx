...
import { addToCart, fetchDishes } from "../../redux/feature/card/cardSlice.js";
...
import type { AppDispatch } from "../../redux/app/store.js";
function Card() {
  const dispatch = useDispatch<AppDispatch>();
  const [products, setProducts] = useState<CartItem[]>([]);
  const [modalOpen, setModalOpen] = useState(false);
  const [modalItem, setModalItem] = useState(null);
  const { card, loading } = useSelector((state: RootState) => state.cart);
  ...
  const handleAddCard = (item: CartItem) => {
    ...
    (toast as any).success("Card is Added!");
  };
      ...
      <div className="grid grid-cols-2 xs:grid-cols-2  sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-3 md:gap-4 lg:gap-5 p-2 sm:p-4">
        {products.map((item: CartItem) => (
          ...
          >
            ...
            >
              <CardImage
                image={`${import.meta.env.VITE_IMAGE}/image/${item?.image}`}
                altData={item.dish}
